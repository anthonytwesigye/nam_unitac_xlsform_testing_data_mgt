...
df_cleaning_log_water <- read_csv("inputs/combined_checks_rundu_water.csv", col_types = cols(sheet = "c", index = "i")) %>% 
...
# raw data
loc_data_water <- "inputs/rundu_water_check_support.csv"
...
# tool
loc_tool_water <- "inputs/rundu_water_form_v2.xlsx"
...
openxlsx::write.xlsx(x = list_of_clean_datasets_water,
                     file = paste0("inputs/clean_rundu_water.xlsx"),
...
df_cleaning_log_building <- read_csv("inputs/combined_checks_rundu_building.csv", col_types = cols(sheet = "c", index = "i")) %>% 
  filter(!adjust_log %in% c("delete_log"), reviewed %in% c("1")) %>%
  mutate(adjust_log = ifelse(is.na(adjust_log), "apply_suggested_change", adjust_log),
         value = ifelse(is.na(value) & str_detect(string = issue_id, pattern = "logic_c_"), "blank", value),
         value = ifelse(type %in% c("remove_survey"), "blank", value),
...
# raw data
loc_data_building <- "inputs/rundu_building_check_support.csv"
...
openxlsx::write.xlsx(x = list_of_clean_datasets_building,
                     file = paste0("inputs/clean_rundu_building.xlsx"),
                     overwrite = TRUE, na.string = "")
...
df_cleaning_log_environmental <- read_csv("inputs/combined_checks_rundu_environmental.csv", col_types = cols(sheet = "c", index = "i")) %>% 
  filter(!adjust_log %in% c("delete_log"), reviewed %in% c("1")) %>%
  mutate(adjust_log = ifelse(is.na(adjust_log), "apply_suggested_change", adjust_log),
         value = ifelse(is.na(value) & str_detect(string = issue_id, pattern = "logic_c_"), "blank", value),
         value = ifelse(type %in% c("remove_survey"), "blank", value),
...
# raw data
loc_data_environmental <- "inputs/rundu_environmental_check_support.csv"
...
openxlsx::write.xlsx(x = list_of_clean_datasets_environmental,
                     file = paste0("inputs/clean_rundu_environmental.xlsx"),
                     overwrite = TRUE, na.string = "")
